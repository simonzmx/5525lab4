...
def train_input_fn(x, y, batch_size, padding_values_x, label_size, num_epochs, max_length):
    ...
    dataset = dataset.map(lambda x1, y1: (x1, tf.one_hot(y1, label_size)))
...
def lstm_model(x, params):
    with tf.name_scope("embedding"):
        ...
        embed = tf.nn.embedding_lookup(w_embed, x, name='embed')  # shape: [batch_size, sequence_len, embedding_dim]

    # reshape embed to shape [sequence_len, batch_size, embedding_dim] for tf.contrib.cudnn_rnn.CudnnLSTM
    ...
    with tf.name_scope("lstm"):
        ...
        # lstm = tf.keras.layers.CuDNNLSTM(units=params['hidden_size'])
        # out = lstm(embed)

    out = output_states_h[-1]

    # out = tf.layers.average_pooling1d(output_states_h[-1], pool_size=params['pool_size'], strides=params['strides'],
    #                                   name='pooling')

    out = tf.layers.dense(out, units=params['n_class'],
...
def model_fn(features, labels, mode, params):
    ...
    loss_op = tf.losses.softmax_cross_entropy(onehot_labels=labels, logits=logits,
                                              reduction=tf.losses.Reduction.MEAN)
    acc_op = tf.metrics.accuracy(labels=true_classes, predictions=predicted_classes, name='acc_op')
...
def main(_):
    ...
    model_dir = os.path.abspath(os.path.join("models", timestamp))
    ...
    model = tf.estimator.Estimator(model_fn, model_dir=model_dir, params=PARAMS)
